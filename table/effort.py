...
def summary_stat(year):
    ...
    pay_nothing = cell(len(effort[effort == 0]) / sample_size)
    pay_much = cell(len(effort[effort >= 0.06]) / sample_size)
    median = cell(effort.quantile(0.5))
    top_vintile = cell(effort.quantile(0.95))

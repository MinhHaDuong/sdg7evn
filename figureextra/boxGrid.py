...
assert set(numericalColumns) <= set(
    survey.select_dtypes(include=["float32", "float64"]).columns
)
...
for i, column in enumerate(numericalColumns):
    for j, yr in enumerate((2010, 2012, 2014)):
        ...
        data = [
            survey.loc[(survey.elec_poor == c) & (survey.year == yr), column].dropna()
            for c in categories
        ]
        plt.boxplot(
            data, labels=categories, showfliers=False, whis=[5, 95], showmeans=True
        )

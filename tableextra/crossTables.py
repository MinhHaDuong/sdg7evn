...
for column in categoricalColumns.drop(
    ["elec_poor", "Q12", "en_subsidy", "block"], axis=1
):
...
for column in categoricalColumns.drop(["elec_poor", "en_subsidy"], axis=1):
    print("")
    print(
        pd.crosstab(survey.loc[:, column], [survey.kwh_last_month == 0], margins=True)
    )
...
for column in categoricalColumns.drop(["elec_poor", "en_subsidy"], axis=1):
    print("")
    print(
        pd.crosstab(survey.loc[:, column], [survey.elec_last_month == 0], margins=True)
    )
...
print(
    pd.crosstab(
        [survey.kwh_last_month == 0], [survey.elec_last_month == 0], margins=False
    )
)
